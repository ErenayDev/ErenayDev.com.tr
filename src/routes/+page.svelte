<script lang="ts">
    ...
    onMount(() => {
        if (shouldUseInViewAnimations) {
            // Observer kullanılıyorsa manuel true yapmıyoruz, observer karar verecek.
            ...
            // Animasyon açık ama observer kapalıysa (Low tier) direkt göster.
            titleInView = true;
            svgInView = true;
        }
    });
            ...
            {#if canUseComplexAnimations}
                ...
                >
                    <div
                        bind:this={svgTarget}
                        use:motion
                        class="absolute bottom-9 left-120 top-1/2 hidden -translate-y-1/2 lg:block"
                    >
                        <SpiralArrow />
                    </div>
                </Motion>
